...
import os
import sys
from plot_utils import plot_all, plot_test_error
...
class Net(object):
    ...
    def backward(self, x, y, learning_rate, momentum, l2_penalty):
        ...
        grad = self.sigmoid_cross_entropy.backward(y, learning_rate, momentum, l2_penalty)
        grad = self.fc2.backward(grad, learning_rate, momentum, l2_penalty)
        grad = self.relu.backward(grad, learning_rate, momentum, l2_penalty)
...
def train_model(train_x, train_y, test_x, test_y, input_dims,
    ...
    for epoch in range(num_epochs):
        ...
        for b in range(num_batches):
            ...
            ce_loss, total_loss = nnet.train(
                x_batch, y_batch, learning_rate, momentum, l2_penalty)
...
def sweep_parameter(param_name, param_values, defaults, train_x, train_y,
    ...
    plot_test_error(
        runs,
        title=f"Test Error vs {param_name}",
        save_path=os.path.join(sweep_dir, "test_error_comparison.png"),
    )
    return runs
...
if __name__ == '__main__':
    ...
    # print("train_y unique values:", np.unique(train_y))
    # print("train_y shape:", train_y.shape)
    # print("train_y dtype:", train_y.dtype)
    # print("train_y values:", train_y)
    # exit()
